"""Zero-shot multimodal retrieval with Chameleon-7B on M-BEIR.
...
``retrieve/chameleon_retrieve.sh`` for the recommended launcher.
"""
import os
import glob
import argparse
...
def extract_embeddings(dataloader, model, accelerator, device):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, disable=not accelerator.is_main_process):
            ...
            pooled = mean_pooling(last_hidden_states, batch['attention_mask'])
