...
Qwen2-VL-7B-Instruct, using activations collected on-the-fly from the
COCO-Caption dataset. See ``sae_train/qwen2_sae_train.sh`` for the launcher.
"""
import os
import argparse
import glob
...
from overcomplete.metrics import r2_score, l0_eps
...
def collect_hidden_states(model, dataloader, device, mask_ids):
    """Run the model on a dataloader and collect last-layer hidden states.

    Tokens corresponding to a leading system-prompt prefix are masked out so
    only data tokens contribute to SAE training.
    """
    hidden_states_pool = []
...
def train_on_pool(sae, optimizer, criterion, pool_hidden, sae_batch_size, device,
    ...
    """Train the SAE for one pass over a pool of hidden states."""
...
def train(args):
    ...
    for epoch in range(args.n_epochs):
        ...
        ckpt_path = os.path.join(args.save_dir, f"sae_qwen2vl_coco_epoch{epoch+1}.pt")
        torch.save({
            "epoch": epoch + 1,
    ...
    final_ckpt = os.path.join(args.save_dir, "sae_qwen2vl_coco_final.pt")
    torch.save({
        "epoch": args.n_epochs,
        "model_state": sae.state_dict(),
        "optimizer_state": optimizer.state_dict(),
