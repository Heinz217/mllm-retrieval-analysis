...
PaliGemma2-3B-Mix-224, using activations collected on-the-fly from the
COCO-Caption dataset. See ``sae_train/paligemma_sae_train.sh`` for the launcher.
"""
import os
import argparse
import glob
...
from collators.eval_collator_paligemma import EvalDataCollator
...
def train(args):
    ...
    for epoch in range(args.n_epochs):
        ...
        ckpt_path = os.path.join(args.save_dir, f"sae_paligemma_coco_epoch{epoch+1}.pt")
        torch.save({
            "epoch": epoch + 1,
    ...
    final_ckpt = os.path.join(args.save_dir, "sae_paligemma_coco_final.pt")
    torch.save({
        "epoch": args.n_epochs,
        "model_state": sae.state_dict(),
        "optimizer_state": optimizer.state_dict(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--d_sae", type=int, default=20736)
