...
Chameleon-7B, using activations collected on-the-fly from the
COCO-Caption dataset. See ``sae_train/chameleon_sae_train.sh`` for the launcher.
"""
import os
import argparse
import glob
...
from collators.eval_collator_chameleon import EvalDataCollatorChameleon as EvalDataCollator
...
def train(args):
    ...
    for epoch in range(args.n_epochs):
        ...
        ckpt_path = os.path.join(args.save_dir, f"sae_chameleon_coco_epoch{epoch+1}.pt")
        torch.save({
            "epoch": epoch + 1,
    ...
    final_ckpt = os.path.join(args.save_dir, "sae_chameleon_coco_final.pt")
    torch.save({
        "epoch": args.n_epochs,
        "model_state": sae.state_dict(),
        "optimizer_state": optimizer.state_dict(),
